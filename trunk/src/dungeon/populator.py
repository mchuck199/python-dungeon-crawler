    ...
    def fill_map_with_creatures(map, filename, min, max):
        ...
        for _ in xrange(min, max):
            ...
            if hasattr(actor, 'swarm'):
                sn, smin, smax = actor.swarm
                for _ in xrange(smin, smax):
                    pos = Populator.game.get_free_adj(actor.pos())
                    if pos == None:
                        break
                    s = Populator.__create_actor(sn, creatures)
                    s.filename = filename
                    s.pop_name = name
                    s.set_pos(pos)
                    Populator.game.add_actor(s, True)
            ...
            if not br:
                ...
                if sn:
                    ...
                    if attr == 'image':
                        ...
                        n = random.randint(0, len(pos) - 1)
                        min, max = pos[n].split(',')
                        actor.min_damage = int(min)
                        actor.max_damage = int(max)
                    elif attr == 'swarm':
                        n, min, max = value.split(',')
                        actor.swarm = n, int(min), int(max)
        return actor
